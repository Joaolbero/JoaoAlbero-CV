...
const translations = {
  ...
  /* HINT DO KONAMI – SEM CARINHA SAFADA */

  "terminal.konami.hint": {
    pt: [
      "Você curte segredos, hein?",
      "Tente a seguinte sequência no teclado (não no terminal):",
      "↑ ↑ ↓ ↓ ← → ← → J V A",
      "Faça isso em qualquer lugar da página para ativar o modo neon especial."
    ],
    en: [
      "You like secrets, huh?",
      "Try the following sequence on your keyboard (not in the terminal):",
      "↑ ↑ ↓ ↓ ← → ← → J V A",
      "Do it anywhere on the page to activate the special neon mode."
    ]
  },
...
function triggerSnakeEasterEgg() {
  ...
  const easterLines = translations["terminal.snake.easter.lines"][currentLang];
  if (body && easterLines) {
    easterLines.forEach((text) => {
  ...
  const konamiHint = translations["terminal.konami.hint"][currentLang];
  if (body && konamiHint) {
    konamiHint.forEach((text) => {
      const div = document.createElement("div");
      div.className = "terminal-line";
      div.textContent = text;
      body.appendChild(div);
